...
class DataGenerationRandomUniformPolicy(DataGeneration):
    ...
    def __init__(self, eval_metric, obj_name, seeder=2, algo_params=None, log_mode='console', checkpoint_log_path=None, 
        ...
        #this block has parameters but i set is_parameterised to False since there are no data generation automatic blocks and so
        #I will never tune this block
        self.is_parametrised = False
        
        self.algo_params = algo_params
        
        if(self.algo_params is None):
            self.algo_params = {'n_samples': 100000} 
    ...
    def learn(self, train_data=None, env=None):
        ...
        if(self.n_jobs == 1):
            ...
            samples = [self.algo_params['n_samples']]
            envs = [starting_env]
        else: 
            samples = []
            envs = []
            for i in range(self.n_jobs):
                samples.append(int(self.algo_params['n_samples']/self.n_jobs))
                envs.append(copy.deepcopy(starting_env))
                envs[i].set_local_prng(new_seeder=starting_env.seeder+i)
                
            samples[-1] = self.algo_params['n_samples'] - sum(samples[:-1])
...
class DataGenerationMEPOL(DataGeneration):
    ...
    def __init__(self, eval_metric, obj_name, seeder=2, algo_params=None, log_mode='console', checkpoint_log_path=None, 
        ...
        if(self.algo_params is None):    
            self.algo_params = {'n_samples': 100000,
                                ...
                                'batch_size': 5000,
                                'create_policy_learning_rate': 0.00025,
                                'state_filter': None, #state_filter equal to None means that i don't do any filtering
                                'zero_mean_start': 1, 
                                'hidden_sizes': [300, 300],
                                'activation': nn.ReLU,
                                'log_std_init': -0.5,
        ...
        if(self.num_traj % self.n_jobs != 0): 
    ...
    def _collect_particles(self, env, policy, num_traj, traj_len):
        ...
        states = np.zeros((num_traj, traj_len+1, len(self.algo_params['state_filter'])), dtype=np.float32)
        ...
        for n_traj in range(num_traj):
            ...
            traj_next_states = states[n_traj, 1:traj_real_len+1, :].reshape(-1, len(self.algo_params['state_filter']))
            if next_states is None:
                next_states = traj_next_states
            else:
                next_states = np.concatenate([next_states, traj_next_states], axis=0)
        
        next_states = next_states[:, self.algo_params['state_filter']]
    ...
    def _compute_entropy(self, behavioral_policy, target_policy, states, actions, num_traj, real_traj_lengths, distances, indices,
        ...
        volumes = (torch.pow(distances[:, self.algo_params['k']], ns) * torch.pow(torch.tensor(np.pi), ns/2)) / G
        # compute entropy
        to_log = (weights_sum/(volumes + self.algo_params['eps'])) + self.algo_params['eps']
        entropy = - torch.sum((weights_sum/self.algo_params['k']) * torch.log(to_log)) + B
    ...
    def _compute_kl(self, behavioral_policy, target_policy, states, actions, num_traj, real_traj_lengths, distances, indices):
        ...
        kl = (1/N)*torch.sum(torch.log(self.algo_params['k']/(N*weights_sum) + self.algo_params['eps']))
    ...
    def _collect_particles_and_compute_knn(self, env, behavioral_policy, num_traj, traj_len):
        ...
        nbrs = NearestNeighbors(n_neighbors=self.algo_params['k']+1, metric='euclidean', algorithm='auto', n_jobs=self.n_jobs)
    ...
    def _policy_update(self, behavioral_policy, target_policy, states, actions, num_traj, traj_len, distances, indices, 
        ...
        self.algo_params['optimizer'].zero_grad()
        ...
        self.algo_params['optimizer'].step()
    ...
    def _mepol(self, env, create_policy):
        ...
        # Set optimizer
        self.algo_params['optimizer'] = self.algo_params['optimizer'](target_policy.parameters(),
                                                                      lr=self.algo_params['learning_rate'])

        # Fixed constants
        ns = len(self.algo_params['state_filter']) 
        B = np.log(self.algo_params['k']) - scipy.special.digamma(self.algo_params['k'])
        full_B = np.log(self.algo_params['full_entropy_k']) - scipy.special.digamma(self.algo_params['full_entropy_k'])
        ...
        params = dict(env=env, behavioral_policy=behavioral_policy, 
                      num_traj=self.algo_params['num_traj']*self.algo_params['full_entropy_traj_scale'], 
                      traj_len=self.algo_params['traj_len'])
        ...
        # Entropy
        params = dict(env=env, behavioral_policy=behavioral_policy, num_traj=self.algo_params['num_traj'], 
                      traj_len=self.algo_params['traj_len'])
        states, actions, real_traj_lengths, next_states, distances, indices = self._collect_particles_and_compute_knn(**params)
    
        with torch.no_grad():
            entropy = self._compute_entropy(behavioral_policy=behavioral_policy, target_policy=behavioral_policy, 
                                            states=states, actions=actions, num_traj=self.algo_params['num_traj'], 
        ...
        if self.algo_params['use_backtracking']:
            original_lr = self.algo_params['learning_rate']
    
        while epoch < self.algo_params['num_epochs']:
            ...
            if self.algo_params['use_backtracking']:
                self.algo_params['learning_rate'] = original_lr
    
                for param_group in self.algo_params['optimizer'].param_groups:
                    param_group['lr'] = self.algo_params['learning_rate']
            ...
            while not kl_threshold_reached:
                ...
                if(not numeric_error and not kl_numeric_error and kl <= self.algo_params['kl_threshold']):
                    # Valid update
                    ...
                    if(self.algo_params['use_backtracking']):
                        ...
                        if(not backtrack_iter == self.algo_params['max_backtrack_try']):
                            target_policy.load_state_dict(last_valid_target_policy.state_dict())
    
                            self.algo_params['learning_rate'] = original_lr/(self.algo_params['backtrack_coeff']**backtrack_iter)
                ...
                if(self.algo_params['use_backtracking'] and (backtrack_iter > 1)):
                    # Just perform at most 1 step using backtracking
                    kl_threshold_reached = True
    
                if(num_off_iters == self.algo_params['max_off_iters']):
                ...
                if kl_threshold_reached:
                    ...
                    with torch.no_grad():
                        entropy = self._compute_entropy(behavioral_policy=last_valid_target_policy, 
                                                        ...
                                                        num_traj=self.algo_params['num_traj'],
    ...
    def _extract_dataset_using_policy(self, env):
        ...
        stacked_dataset: This is a list and it represents the extracted dataset: each element of the list is a sample from the
                         enironment containing the current state, the current action, the reward, the next state, the absorbing 
    ...
    def learn(self, train_data=None, env=None):
        ...
        if(self.algo_params['state_filter'] is None):
            self.algo_params['state_filter'] = np.arange(env.observation_space.shape[0]).tolist()
                    
        def create_policy(is_behavioral=False):         
            policy = self.gaussian_policy(num_features=len(self.algo_params['state_filter']), 
                                          hidden_sizes=self.algo_params['hidden_sizes'], action_dim=env.action_space.shape[0],
                                          activation=self.algo_params['activation'], 
                                          log_std_init=self.algo_params['log_std_init'])
        
            if(is_behavioral and self.algo_params['zero_mean_start']):
                optimizer = torch.optim.Adam(policy.parameters(), lr=self.algo_params['create_policy_learning_rate'])

                for _ in range(self.algo_params['train_steps']):
                    optimizer.zero_grad()
                    
                    states = [env.sample_from_box_observation_space()[:len(self.algo_params['state_filter'])] 
